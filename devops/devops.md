### DevOps
- [Mac setup for developers](https://github.com/sb2nov/mac-setup)
- [The complete process of developing an app](https://12factor.net/)
- [Brack's hosting demo](https://github.com/Kedirech/Hosting-demo-postgres)
- [Designing Large Scale Systems](https://github.com/donnemartin/system-design-primer)


### SaaS Security Tips
- [Security for Startups](https://github.com/forter/security-101-for-saas-startups)


### What if your domain gets stolen?
- [Here's how one girl successfully handled it](https://medium.freecodecamp.com/hackers-stole-my-website-and-i-pulled-off-a-30-000-sting-operation-to-get-it-back-143d43ee3742#.g7ayyl735)


### Todo list:
- [rancher](http://rancher.com/)
- [jenkins](https://wiki.jenkins-ci.org/display/JENKINS/Installing+Jenkins)
- [ansible setup](https://serversforhackers.com/an-ansible-tutorial)
- [certbot and node](https://startupnextdoor.com/how-to-obtain-and-renew-ssl-certs-with-lets-encrypt-on-node-js/)
- [additional dotfiles](https://github.com/paulirish/dotfiles)
- [dockerizing node app](https://nodejs.org/en/docs/guides/nodejs-docker-webapp/)
- [alpine node](https://github.com/mhart/alpine-node)
- [docker compose](https://github.com/b00giZm/docker-compose-nodejs-examples/blob/master/05-nginx-express-redis-nodemon/README.md)
